...
class canvasClass {
     ...
     */
    fillArray(arr, colorArray, w = Math.floor(this.canvas.width / arr[0].length), h = Math.floor(this.canvas.height / arr.length), xOffset = 0, yOffset = 0) {
        let abstand = 0;
        for (let y = 0; y < arr.length; y++) {
            for (let x = 0; x < arr[0].length; x++) {
                ...
                if (lvl.coins[xyToI(x, y, spalten)]) {
                    this.fillCircle(Math.round(x * w + abstand + xOffset + w / 2), Math.round(y * h + abstand + yOffset + h / 2), Math.round(h / 5), "yellow");
                }
                if (lvl.coins[xyToI(x, y, spalten)] && lvl.grid[y][x] == 1) {
                    lvl.coinAnzahl -= 1;
                    lvl.coins[xyToI(x, y, spalten)] = false;
                }
            }
        }
    }
